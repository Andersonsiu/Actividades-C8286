...
class scheduler:
    def __init__(self, process_switch_behavior, io_done_behavior, io_length, scheduling_policy, quantum):
        self.proc_info = {}
    ...
    def new_process(self):
        ...
        self.proc_info[proc_id]['remaining_time'] = 0  # For STCF
        return proc_id
    ...
    def load_program(self, program):
        ...
        for line in program.split(','):
            ...
            if opcode == 'c':  # compute
                ...
                exit(1)
            self.proc_info[proc_id]['remaining_time'] += num  # For STCF
        return
    ...
    def load(self, program_description):
        ...
        for i in range(num_instructions):
            if random.random() < chance_cpu:
                ...
                self.proc_info[proc_id][PROC_CODE].append(DO_IO)
                self.proc_info[proc_id][PROC_CODE].append(DO_IO_DONE)
            self.proc_info[proc_id]['remaining_time'] += 1  # For STCF
        return
    ...
    def next_proc(self, pid=-1):
        if self.scheduling_policy == SCHED_RR:
            return self.round_robin_next_proc()
        elif self.scheduling_policy == SCHED_SJF:
            return self.sjf_next_proc()
        elif self.scheduling_policy == SCHED_STCF:
            return self.stcf_next_proc()
        else:
            return self.fifo_next_proc()

    def fifo_next_proc(self, pid=-1):
        if pid != -1:
            self.curr_proc = pid
    ...
    def sjf_next_proc(self):
        shortest_time = float('inf')
        next_proc = -1
        for pid in range(len(self.proc_info)):
            if self.proc_info[pid][PROC_STATE] == STATE_READY:
                if self.proc_info[pid]['remaining_time'] < shortest_time:
                    shortest_time = self.proc_info[pid]['remaining_time']
                    next_proc = pid
        if next_proc != -1:
            self.curr_proc = next_proc
            self.move_to_running(STATE_READY)
        return

    def stcf_next_proc(self):
        shortest_time = float('inf')
        next_proc = -1
        for pid in range(len(self.proc_info)):
    ...
    def round_robin_next_proc(self):
    ...
    def run(self):
        ...
        while self.get_num_active() > 0:
                ...
                if clock_tick in self.io_finish_times[pid]:
                    ...
                    if self.io_done_behavior == IO_RUN_IMMEDIATE:
                        ...
                        if self.process_switch_behavior == SCHED_SWITCH_ON_END and self.get_num_runnable() > 1:
                            # esto significa que el proceso que emitió el io debe ejecutarse
                            self.next_proc(pid)
                        if self.get_num_runnable() == 1:
                            # esto es lo único que hay que ejecutar: así que ejecútalo
                            self.next_proc(pid)
                    self.check_if_done()

            # si el proceso actual es RUNNING y tiene una instrucción, ejecutarla
            instruction_to_execute = ''
            if self.proc_info[self.curr_proc][PROC_STATE] == STATE_RUNNING and \
                    len(self.proc_info[self.curr_proc][PROC_CODE]) > 0:
                instruction_to_execute = self.proc_info[self.curr_proc][PROC_CODE].pop(0)
                self.proc_info[self.curr_proc]['remaining_time'] -= 1
                cpu_busy += 1

            # OUTPUT: imprimir lo que están haciendo todos
            if io_done:
            ...
            # Salida de CPU aquí: si no se ejecuta ninguna instrucción, salida un espacio, de lo contrario, un 1
            ...
            # si esta es una instrucción de inicio de E/S, cambiar a estado de espera
            # y agregar una finalización de io en el futuro
            ...
            # ENDCASE: verificar si lo que se está ejecutando actualmente está fuera de instrucciones
            self.check_if_done()
...
parser.add_option('--policy', default='FIFO', help='scheduling policy: FIFO, SJF, STCF, RR', action='store', type='string', dest='scheduling_policy')
parser.add_option('--quantum', default=1, help='quantum length for RR', action='store', type='int', dest='quantum')
...
assert(options.scheduling_policy in [SCHED_FIFO, SCHED_SJF, SCHED_STCF, SCHED_RR, SCHED_SQMS, SCHED_MQMS])

s = scheduler(options.process_switch_behavior, options.io_done_behavior, options.io_length, options.scheduling_policy, options.quantum)
